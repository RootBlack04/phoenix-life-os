...
try {
  ...
  if (process.env.PHOENIX_GOALS_ONLY === "1") {
    ...
    await region.getByRole("link", { name: "View all goals", exact: true }).click();
    ...
    await historyCard.getByText("100% progress", { exact: true }).waitFor();
    ...
    await open("/"); await card.waitFor(); await card.getByText("100%", { exact: true }).waitFor();
